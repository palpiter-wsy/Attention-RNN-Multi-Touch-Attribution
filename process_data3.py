...
def process_data(data, seq_length=20):
    ...
    cat(data, 'age', "leng_type", s=',')
    cat(data, 'gender', "leng_type", s=',')
    cat(data, 'country', "leng_type", s=',')
    ...
    data_new.age = data_new.age.map(lambda x: most_fre(x))
    data_new.gender = data_new.gender.map(lambda x: most_fre(x))
    data_new.country = data_new.country.map(lambda x: most_fre(x))
    ...
    # 输出1：训练集时间
    time_decay_tr = time_decay[tr_idx]  # 占0.8
    # 输出2：测试集时间
    time_decay_te = time_decay[te_idx]  # 占0.2
    ...
    t.fit_on_texts(text)
    # vocabulary size
    vocab_size = len(t.word_index) + 3  # 8
    print(vocab_size)
    ...
    data_lr = data_new.loc[:, ['marketing_area', 'tier', 'customer_type', 'age', 'gender', 'country']]
    train_cats(data_lr)
    ...
    scaler = MinMaxScaler()
    scaler.fit(X_tr_lr)
    X_tr_lr[['marketing_area', 'tier', 'customer_type', 'age', 'gender', 'country']] = scaler.fit_transform(
        X_tr_lr[['marketing_area', 'tier', 'customer_type', 'age', 'gender', 'country']])
    X_te_lr[['marketing_area', 'tier', 'customer_type', 'age', 'gender', 'country']] = scaler.transform(
        X_te_lr[['marketing_area', 'tier', 'customer_type', 'age', 'gender', 'country']])
    # 输出13：用户属性模块的哪些字段
    categorical_vars = data_lr.columns[0:6]
