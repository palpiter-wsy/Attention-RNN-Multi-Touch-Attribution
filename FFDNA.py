...
class FFDNA(object):
    ...
    '''
    def train_model(self,save_name, loss='binary_crossentropy',opt='adam',metrics=['accuracy']):#交叉熵损失函数，Adam优化算法；metrics=accuracy评估方法
        ...
        self.history = self.model.fit([self.X_tr,self.s0,self.time_decay_tr,self.X_tr_lr.iloc[:,0],self.X_tr_lr.iloc[:,1],
              ...
              ], self.Y_train, epochs=self.epochs, batch_size=self.batch_size,verbose=2)#2为每个epoch输出一行记录
...
if __name__ == '__main__':
    ...
    n_channels = len(channels) #4
    vocab_size = n_channels +1  #5
    epochs = 120
